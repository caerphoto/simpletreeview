...
TreeDataError.prototype=new Error();TreeDataError.prototype.constructor=TreeDataError;function appendText(node,text){return node.appendChild(document.createTextNode(text));}
function setChildrenState(node,state){_(node.children).each(function(child){child.state=state;setNodeElementStateClass(child);setChildrenState(child,state);});return state;}
...
el.setAttribute('data-node-id',this.id);el.setAttribute('data-value',this.value);el.className+=' stv-node';label.className='stv-label';if(this.tree.__options.HTMLLabels){label.innerHTML=this.label;}else{appendText(label,this.label);}
...
node.tree.__nodeHash[node.id]=node;return node;}};TreeNode.prototype={toString:function(){return this.value;},valueOf:function(){return this.value;},labelOrValueMatches:function(lowerCaseTerm){var label=this.label.toLowerCase();var value;if(label.indexOf(lowerCaseTerm)!==-1){return true;}
...
_.each(root.children,function(child){nodes=nodes.concat(findNodes(child,lowerCaseTerm));});return nodes;}
function toggleNodeExpansion(node,expanded){if(expanded){node.elements.$el.removeClass('stv-collapsed');node.elements.$el.addClass('stv-expanded');}else{node.elements.$el.toggleClass('stv-collapsed');node.elements.$el.toggleClass('stv-expanded');}
...
throw new TypeError('Expected string or array when setting selection, but got '+(typeof selection));},getMatching:function(){return this.matchingNodes;},__createFilterElements:function(){var elFilterControls=this.elFilterControls=document.createElement('div');var elMatchCount=this.elMatchCount=document.createElement('span');var elFilter=this.elFilter=document.createElement('input');var label=document.createElement('label');var elSelectMatching=this.elSelectMatching=document.createElement('button');elFilterControls.className='stv-filter-controls';elFilter.className='stv-filter-input';elFilter.type='search';appendText(label,STRINGS.searchLabel);label.className='stv-filter-label';elMatchCount.className='stv-match-count';appendText(elMatchCount,STRINGS.allMatch);elSelectMatching.className='stv-select-matching';appendText(elSelectMatching,'Select');elFilterControls.appendChild(label);elFilterControls.appendChild(elFilter);elFilterControls.appendChild(elMatchCount);elFilterControls.appendChild(elSelectMatching);this.$elFilter=$(this.elFilter);this.$elSelectMatching=$(this.elSelectMatching);},__applyFilter:function(term){var matchingNodes;var matchText=STRINGS.allMatch;this.__$el.toggleClass('stv-filtering',!!term);this.__$rootElement.find('.stv-filter-match').removeClass('stv-filter-match');this.__$rootElement.find('.stv-filter-descendant-match').removeClass('stv-filter-descendant-match');if(!term){this.matchingNodes=[];this.elMatchCount.innerHTML=matchText;return;}
matchingNodes=findNodes(this.getData(),term.toLowerCase());_(matchingNodes).each(function(node){ensureElements(node);node.elements.$el.addClass('stv-filter-match');node.elements.$el.parentsUntil(this.__$el,'.stv-node').addClass('stv-filter-descendant-match');});this.matchingNodes=matchingNodes;matchText=STRINGS.someMatch+
(matchingNodes.length!==1?'es':'');this.elMatchCount.innerHTML=matchText.replace('#',matchingNodes.length);},__selectMatching:function(){_(this.matchingNodes).invoke('select');},__setEventHandlers:function(){var tree=this;this.__$rootElement.on('click','.stv-checkbox, .stv-label',function(){var nodeId=this.parentNode.getAttribute('data-node-id');var node=tree.nodeWithId(nodeId);if(!node){throw new Error('Unable to handle click on element with node id '+nodeId+': node not found');}
...
this.$elFilter.on('keyup input',function(){var term=this.value;var delay=tree.__options.filterDelay;if(delay===false){tree.__applyFilter(term);}else{clearTimeout(tree.__filterTimeout);tree.__filterTimeout=setTimeout(function(){tree.__applyFilter(term);},500);}});this.$elSelectMatching.on('click',function(){tree.__selectMatching();});},render:function(recurseDepth){if(!this.__el||!isElement(this.__el)){throw new TypeError('Unable to render tree with no valid element');}
recurseDepth=_.isNumber(recurseDepth)?recurseDepth:-1;createElements.call(this.__root,recurseDepth);if(this.elFilterControls){this.__el.appendChild(this.elFilterControls);}
