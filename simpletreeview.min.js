...
(function(){var self=this;var D=self.document;var UNSELECTED=0;var PARTIAL=1;var SELECTED=2;function NodeLocationError(loc){this.loc=loc||[];this.name='NodeLocationError';this.message=' is not a valid node location.';this.toString=function(){return this.loc.join(' ')+this.message;};}
...
this.tree=tree;this.parent=parent;this.state=_(nodeAttrs).has('state')?nodeAttrs.state:UNSELECTED;this.id=_.uniqueId('STV_');this.elements=null;if(_(nodeAttrs).has('children')&&_(nodeAttrs.children).isArray()&&nodeAttrs.children.length){this.children=_(nodeAttrs.children).map(function(child){return new TreeNode(child,this,tree);},this);}else{this.children=[];}
...
if(o.element){this.setElement(o.element);}};function copyNode(node){return{label:node.label,value:node.value,parent:node.parent,state:node.state,children:_(node.children).map(copyNode)};}
