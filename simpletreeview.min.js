...
_(TreeNode.prototype).extend({labelOrValueMatches:function(lowerCaseTerm){var label=this.label.toLowerCase();var value;if(label.indexOf(lowerCaseTerm)!==-1){return true;}
value=this.value.toLowerCase();if(value.indexOf(lowerCaseTerm)!==-1){return true;}
return false;},__createElements:function(recurseDepth,appendToParentList){var el;var label=D.createElement('label');var checkbox=D.createElement('span');var expander=D.createElement('span');var childList;if(this.parent){el=D.createElement('li');el.className='stv-child-node';}else{el=this.tree.__rootElement;el.className='stv-root-node';}
...
if(node.elements.$el.hasClass('stv-expanded')&&node.children.length>0){_(node.children).each(function(child){if(!child.elements){child.__createElements(0,true);}});}}
...
this.$elFilter.on('keyup',function(){var term=this.value;clearTimeout(tree.__filterTimeout);tree.__filterTimeout=setTimeout(function(){tree.__applyFilter(term);},500);});},render:function(recurseDepth){if(!this.__el||!isElement(this.__el)){throw new TypeError('Unable to render tree with no valid element');}
recurseDepth=_.isNumber(recurseDepth)?recurseDepth:-1;this.__root.__createElements(recurseDepth);if(this.elFilter){this.__el.appendChild(this.elFilter);}
this.__el.appendChild(this.__rootElement);}});SimpleTreeView.NodeLocationError=NodeLocationError;SimpleTreeView.TreeDataError=TreeDataError;SimpleTreeView.UNSELECTED=UNSELECTED;SimpleTreeView.PARTIAL=PARTIAL;SimpleTreeView.SELECTED=SELECTED;global.SimpleTreeView=SimpleTreeView;if(typeof define==='function'&&define.amd){define(['underscore','jquery'],function(){return SimpleTreeView;});}}).call(this);